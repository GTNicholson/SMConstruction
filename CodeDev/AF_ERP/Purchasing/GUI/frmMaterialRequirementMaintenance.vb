...
Public Class frmMaterialRequirementMaintenance
  ...
  Public Shared Sub OpenFormAsMDIChild(ByRef rMDI As Windows.Forms.Form, ByRef rRTISGlobal As AppRTISGlobal, ByRef rDBConn As RTIS.DataLayer.clsDBConnBase, ByVal vSalesOrderPhaseID As Integer, ByVal vConsoleOptionView As ePOConsoleOption)
    Dim mfrm As frmMaterialRequirementMaintenance = Nothing

    mfrm = GetFormIfLoaded(rMDI)
    If mfrm Is Nothing Then
      mfrm = New frmMaterialRequirementMaintenance
  ...
  Private Shared Function GetFormIfLoaded(ByVal vMdi As Windows.Forms.Form) As frmMaterialRequirementMaintenance
    Dim mfrmWanted As frmMaterialRequirementMaintenance = Nothing
    Dim mFound As Boolean = False
    Dim mfrm As frmMaterialRequirementMaintenance

    'Check if exisits already
    If sActiveForms Is Nothing Then sActiveForms = New Collection
    For Each mfrm In sActiveForms
      If TypeOf mfrm Is frmMaterialRequirementMaintenance And mfrm.MdiParent Is vMdi Then
        mfrmWanted = mfrm
        mFound = True
        Exit For
      End If
  ...
  Private Sub frmMaterialRequirementMaintenances_Load(sender As Object, e As EventArgs) Handles Me.Load
  ...
  Private Sub RepositoryItemPopupContainerEditOrderedQty_Popup(sender As Object, e As EventArgs) Handles RepositoryItemPopupContainerEditOrderedQty.ButtonClick
    ...
    If mRow IsNot Nothing Then
      mwhere = String.Format("StockItemID ={0} and POStatus not in (6) and MaintenanceWorkOrderID = {1}", mRow.StockItem.StockItemID, mRow.MaterialRequirement.ObjectID)
  ...
  Private Sub frmMaterialRequirementMaintenance_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
    Try
  ...
  Private Sub frmMaterialRequirementMaintenance_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
