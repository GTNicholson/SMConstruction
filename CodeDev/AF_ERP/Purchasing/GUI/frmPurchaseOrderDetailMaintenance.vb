...
Public Class frmPurchaseOrderDetailMaintenance
  ...
  Public Shared Sub OpenFormAsMDIChild(ByRef rParentForm As Windows.Forms.Form, ByRef rDBConn As RTIS.DataLayer.clsDBConnBase, ByRef rUserSession As clsRTISUser, ByRef rRTISGlobal As clsRTISGlobal, ByVal vPrimaryKeyID As Integer, ByVal vFormMode As eFormMode, ByVal vPODetailOption As ePODetailOption)
    Dim mfrm As frmPurchaseOrderDetailMaintenance = Nothing
    ...
    If mfrm Is Nothing Then
      mfrm = New frmPurchaseOrderDetailMaintenance
  ...
  Private Shared Function GetFormIfLoaded(ByVal vPrimaryKeyID As Integer) As frmPurchaseOrderDetailMaintenance
    Dim mfrmWanted As frmPurchaseOrderDetailMaintenance = Nothing
    Dim mFound As Boolean = False
    Dim mfrm As frmPurchaseOrderDetailMaintenance
  ...
  Private Sub LoadCombos()
    ...
    grdPOIMaintenanceWorkOrder.DataSource = pFormController.MaintenanceWorkOrders
    grdPurchaseOrderItems.DataSource = pFormController.PurchaseOrder.PurchaseOrderItems ''.POItemsMinusAllocatedItem
  ...
  Public Sub LoadPOItemAllocationCombo()
    ...
    For Each mWOI As dmMaintenanceWorkOrder In pFormController.MaintenanceWorkOrders
      If mWOI IsNot Nothing Then
        mValueItems.AddNewItem(mWOI.MaintenanceWorkOrderID, mWOI.MaintenanceWorkOrderNo)
      End If
    Next

    clsDEControlLoading.LoadGridLookUpEdit(grdPOIMaintenanceWorkOrder, gvPOIMaintenanceWorkOrders.Columns("MaintenanceWorkOrderID"), mValueItems)
  End Sub
        ...
        Select Case .MaterialRequirementTypeMaintenanceID
          ...
          Case ePOMaterialRequirementType.Sencillo
            ...
            If pFormController.MaintenanceWorkOrder IsNot Nothing Then
              btnSelectWorkOrder.Text = pFormController.MaintenanceWorkOrder.MaintenanceWorkOrderNo
              txtProjectNameWO.Text = pFormController.MaintenanceWorkOrder.Description
              txtCustomerNameWO.Text = pFormController.MaintenanceWorkOrder.Machinery.Description
              dteRequiredDateWO.EditValue = pFormController.MaintenanceWorkOrder.PlannedDate
            End If
    ...
    With pFormController.PurchaseOrder
      ...
      Select Case .MaterialRequirementTypeMaintenanceID
        Case ePOMaterialRequirementType.Sencillo
          ...
          If pFormController.MaintenanceWorkOrder IsNot Nothing Then
            .RefMatType = pFormController.MaintenanceWorkOrder.MaintenanceWorkOrderNo
          End If
      ...
      gvPOIMaintenanceWorkOrders.CloseEditor()
      grdPOIMaintenanceWorkOrder.Update()
  ...
  Private Sub grpWorkOrderType_CustomButtonChecked(sender As Object, e As BaseButtonEventArgs) Handles grpWorkOrderType.CustomButtonChecked
    Try
      ...
      If pIsActive Then
        ...
        gvMaintenanceWorkOrders.CloseEditor()
        gvMaintenanceWorkOrders.UpdateCurrentRow()
        UpdateObject()
        CheckSave(False)
        ...
        Select Case e.Button.Properties.Tag
          ...
          Case ePOMaterialRequirementType.Inventario
            If e.Button.Properties.IsChecked Then
              ...
              grdPOIMaintenanceWorkOrder.DataSource = pFormController.MaintenanceWorkOrders
              grdPOIMaintenanceWorkOrder.RefreshDataSource()
  ...
  Private Sub btnSelectWorkOrder_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles btnSelectWorkOrder.ButtonClick
    Try
      Dim mPicker As clsPickerWorkOrderMaintenance
      ...
      mPicker = New clsPickerWorkOrderMaintenance(mMaintenanceWorkOrders, pFormController.DBConn)

      mMaintenanceWorkOrder = frmMaintenanceWorkOrderPicker.OpenPickerSingle(mPicker)
      ...
      If mMaintenanceWorkOrder IsNot Nothing Then
        ...
        pFormController.MaintenanceWorkOrders.Add(mMaintenanceWorkOrder)
        pFormController.MaintenanceWorkOrder = mMaintenanceWorkOrder

        If pFormController.PurchaseOrder.PurchaseOrderAllocations.IndexFromMaintenanceWorkOrderID(mMaintenanceWorkOrder.MaintenanceWorkOrderID) = -1 Then
          mPOAllocation = New dmPurchaseOrderAllocation
          mPOAllocation.MaintenanceWorkOrderID = mMaintenanceWorkOrder.MaintenanceWorkOrderID
  ...
  Private Sub gvPOIMaintenanceWorkOrders_CustomUnboundColumnData(sender As Object, e As CustomColumnDataEventArgs) Handles gvPOIMaintenanceWorkOrders.CustomUnboundColumnData
    Dim mRow As dmPurchaseOrderItemAllocation
    Dim mWOInfo As dmMaintenanceWorkOrder
      ...
      If mRow IsNot Nothing Then
        ...
        Select Case e.Column.Name
          ...
          Case gcItemQuantity.Name
            ...
            If pFormController.MaintenanceWorkOrders IsNot Nothing Then
              If e.IsGetData Then
                mWOInfo = pFormController.MaintenanceWorkOrders.ItemFromKey(mRow.MaintenanceWorkOrderID)
                e.Value = mRow.Quantity
                If mWOInfo IsNot Nothing Then


                  mRow.ItemRef = mWOInfo.MaintenanceWorkOrderNo
                  mRow.ItemRef2 = mWOInfo.Description
                  mRow.ProjectRef = mWOInfo.Machinery.Description
  ...
  Private Sub btnSelectMultipleWO_Click(sender As Object, e As EventArgs) Handles btnSelectMultipleWO.Click
    Try
      ...
      Dim mSelectedMaintenanceWorkOrders As colMaintenanceWorkOrders
      ...
      For Each mPOA In pFormController.PurchaseOrder.PurchaseOrderAllocations ''.POItemsMinusAllocatedItem
        If mPOA.MaintenanceWorkOrderID > 0 Then
          mMaintenanceWorkOrder = mMaintenanceWorkOrders.ItemFromKey(mPOA.MaintenanceWorkOrderID)

          If Not mPicker.SelectedObjects.Contains(mMaintenanceWorkOrder) Then
            mPicker.SelectedObjects.Add(mMaintenanceWorkOrder)
          End If
        End If
      Next

      frmMaintenanceWorkOrderPicker.OpenPickerMulti(mPicker, True)
      For Each mSelectedItem In mPicker.SelectedObjects
        If mSelectedItem IsNot Nothing Then
          mPurchaseOrderAllocation = pFormController.PurchaseOrder.PurchaseOrderAllocations.ItemFromMaintenanceWorkOrderID(mSelectedItem.MaintenanceWorkOrderID)
          If mPurchaseOrderAllocation Is Nothing Then
            mPurchaseOrderAllocation = New dmPurchaseOrderAllocation
            mPurchaseOrderAllocation.MaintenanceWorkOrderID = mSelectedItem.MaintenanceWorkOrderID
            pFormController.PurchaseOrder.PurchaseOrderAllocations.Add(mPurchaseOrderAllocation)
          End If
        End If
      Next
      mSelectedMaintenanceWorkOrders = New colMaintenanceWorkOrders
      For Each mWOI As dmMaintenanceWorkOrder In mPicker.SelectedObjects
        mSelectedMaintenanceWorkOrders.Add(mWOI)
      Next
      ...
      For mindex As Integer = pFormController.PurchaseOrder.PurchaseOrderAllocations.Count - 1 To 0 Step -1
        ...
        mMaintenanceWorkOrder = mSelectedMaintenanceWorkOrders.ItemFromKey(mPurchaseOrderAllocation.MaintenanceWorkOrderID)
