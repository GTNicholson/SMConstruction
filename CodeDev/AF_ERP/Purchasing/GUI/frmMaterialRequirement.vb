...
Public Class frmMaterialRequirement
  ...
  '    gvMaterialRequirements.RefreshData()
  '    MsgBox(mCount & " Item(s) from stock quantity updated.")
  '  Catch ex As Exception
  '    If clsErrorHandler.HandleError(ex, clsErrorHandler.PolicyUserInterface) Then Throw
  '  End Try
  'End Sub
  ...
  '    grdOnOrderOSQty.DataSource = mRow.POCOItemAllocationInfos
  '  Else
  ...
  '      If mPO.Status <> ePurchaseOrderDueDateStatus.InProcess Or mPO.PurchaseOrderID = 0 Then
  ...
  '  If mRow IsNot Nothing Then
  '    If mRow.StockItemTransactions.Count = 0 Then
  '      pFormController.LoadTransactions(mRow)
  '    End If
  '    grdStockitemTrans.DataSource = mRow.StockItemTransactions
  '  Else
